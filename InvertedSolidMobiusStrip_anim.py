...
# rotation around the y-axiss
O = np.array([0.0, 0.0, 0.0])
A = np.array([0.0, 10.0, 0.0])
Rot = AffineRotationMatrix(3*pi/4, O, A)


# the Solid Mobius strip implicit equation
def f(x, y, z, a, b):
    return ((
...
# the satellte (spherical) curve, used to rotate the camera
...
for i, t in enumerate(t_):
    ...
    pltr.set_position(center + satellite(t, 7, 3*pi/4, 4))
...
os.system(
    "gifski --frames=zzpic*.png --fps=10 -o InvertedSolidMobiusStrip.gif"    
)
