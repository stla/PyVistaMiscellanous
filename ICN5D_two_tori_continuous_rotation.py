...
def f(x, y, z, a):
    return (
        (np.sqrt(x*x + (y*sin(a))**2) - 2)**2 
            + (np.sqrt(z*z + (y*cos(a))**2) - 2)**2 -0.5**2
    )
...
os.system(
    "gifski --frames=zzpic*.png --fps=9 -o ICN5D_two_tori_continuous_rotation.gif"
)
