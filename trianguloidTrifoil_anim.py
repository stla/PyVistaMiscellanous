...
U, V = np.meshgrid(np.linspace(-pi, pi, 250), np.linspace(-pi, pi, 250))
X, Y, Z = f(U, V)
...
pos0, quats = satellite_motion(180, 24)
...
for i, q in enumerate(quats):
    ...
    pltr.camera.zoom(1)
    pltr.add_mesh(
        mesh, smooth_shading=True, specular=5, cmap="turbo", log_scale=False,
        show_scalar_bar=False, flip_scalars=False 
    )
    pltr.show(screenshot=pngname)

os.system(
    "magick convert -dispose previous -delay 1x9 zzpic*.png trianguloidTrifoil.gif"    
)
