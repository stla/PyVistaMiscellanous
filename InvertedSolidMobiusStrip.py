...
def params(alpha, gamma, mu):
    ...
    ratio = (mu - gamma) * ((alpha - gamma) * (mu + gamma) + theta) / ((alpha + gamma) * (mu - gamma) + theta) / (alpha - gamma)
    R = 1/ratio * gamma * gamma / ((alpha - gamma) * (mu - gamma) + theta) * (mu - gamma) / ((alpha + gamma) * (mu - gamma) + theta)
    omegaT = omega - (beta * beta * (omega - gamma)) / ((alpha - gamma) * (mu + omega) - beta * beta) / ((alpha + gamma) * (omega - gamma) + beta * beta)
...
X, Y, Z = np.mgrid[(-1.5):1.5:150j, (-1.8):1.8:150j, (-0.8):0.8:150j]
...
#s = np.array([2.4/2.6, 2.8/3.3, 2/1.3])
#points2 = np.apply_along_axis(lambda M: s*M+newmesh.center, 1, mesh.points)
dists = np.linalg.norm(mesh.points, axis=1)
newmesh["dist"] = dists 

pltr = pv.Plotter(window_size=[512, 512])
pltr.set_focus(newmesh.center)
pltr.set_position(newmesh.center - np.array([0.0, 0.0, 6]))
pltr.add_background_image("SpaceBackground.png")
pltr.add_mesh(newmesh, smooth_shading=True, cmap="nipy_spectral", specular=25)
pltr.show()
